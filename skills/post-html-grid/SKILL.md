---
name: post-html-grid
description: Use when the user wants an HTML dashboard, report, comparison board, or "grid" deployed somewhere viewable. POSTs the HTML to a running `pergam` server and returns a `http://localhost:1111/{id}/view` URL (or whatever the configured public URL is). Supports versioning — same id with version++ when the user iterates on a previous grid. Triggered by phrases like "publish/post/deploy this as a grid", "armá un grid HTML", "show this as a viewable HTML grid", "actualizá el grid", "v2 de", "iterá sobre", or any request to output HTML that should be browseable.
---

# post-html-grid

Publish a self-contained HTML document to a running [`pergam`](https://github.com/diesilveira/pergam) server and return a shareable URL. Grids are **versioned and immutable**: every POST either creates a new logical grid (random 8-hex `id`, `version=1`) or bumps the version of an existing one (same `id`, `version` incremented).
...
2. **Decide: new grid or new version?** See [§ Versioning](#versioning).

3. **Pick `grid_type`** from `plan | investigacion | informe | reporte | otro`. See [§ Grid types](#grid-types).
...
5. **Build the HTML** as a complete, self-contained document (see HTML guidelines below). Write it to a temp file so the JSON payload escapes cleanly — do NOT try to embed multi-line HTML directly in `curl -d`.
   ...
   python3 - <<'PY'
   import json, pathlib
   html = pathlib.Path("/tmp/grid.html").read_text()
   pathlib.Path("/tmp/grid.payload.json").write_text(json.dumps({
       "title":     "My grid title",
       "html":      html,
       "grid_type": "plan",                # required
...
6. **POST it:**
   ...
   curl -sS -X POST http://localhost:1111/grid \
        ...
        --data-binary @/tmp/grid.payload.json
   ```
   Response: `{"id":"abc12345", "version": 2, "view_url":"http://localhost:1111/abc12345/view", "title":"...", "grid_type":"...", "author":"..."}`
...
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| "armá un grid nuevo de X", "publish a fresh report"                  | **New `id`**. Omit `id` in the POST.                                          |
| "actualizá el grid", "nueva versión", "v2", "siguiente versión"      | **Same `id`, version++**. Resolve `id` from context — usually the most recent `view_url` posted in this conversation. |
...
| Ambiguous and there is a recent grid in the conversation             | Default to **new version** of the most recent.                                |
| Ambiguous and no recent grid                                         | Default to **new `id`**.                                                      |
...
When iterating (new version of an existing grid), **keep the same type as the previous version** unless the content shifts categorically.

## HTML guidelines

- Complete document: `<!doctype html><html lang="...">...</html>`.
- Inline all CSS. **No external fonts.** **No external JS** except Mermaid (see below).
- Use CSS Grid or flexbox for the actual "grid" layout.
...
**Short grid** (3–10 cards, dashboard, PR list, comparison board):
...
If content has 5+ logical sections, diagrams, or schemas → long-form template. Otherwise → short grid.
...
| ------ | --------------------- | -------------------------------------------------------------------- |
| POST   | `/grid`               | Body `{title, html, grid_type, author, id?}`. Returns `{id, version, view_url, title, grid_type, author}`. |
...
- **Container restarts in a loop, logs show "could not connect to server"** → the pergam server can't reach its Postgres. Common cause: the database hostname in `GRID_DB_URL` is unreachable from inside the container. If using the bundled compose, `docker compose restart` after the db is ready.
- **400 "grid_type must be one of …"** → fix the payload to use one of `plan | investigacion | informe | reporte | otro`.
- **400 "Missing author"** → always pass `author` (Claude session email).
- **405 on DELETE** → expected. Grids are immutable. Post a new version to supersede.
